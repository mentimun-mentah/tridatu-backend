...
class TestVariant(OperationTest):
    ...
    def test_validation_create_variant(self,client):
        ...
        for x in response.json()['detail']:
            ...
            if x['loc'][-1] == 'va1_product_id': assert x['msg'] == 'ensure this value has at least 1 characters'
        ...
        assert response.status_code == 422
        for x in response.json()['detail']:
            if x['loc'][-1] == 'va1_id': assert x['msg'] == 'ensure this value has at least 1 characters'
            if x['loc'][-1] == 'va1_option': assert x['msg'] == 'ensure this value has at least 1 characters'
            if x['loc'][-1] == 'va1_price': assert x['msg'] == 'ensure this value has at least 1 characters'
            if x['loc'][-1] == 'va1_stock': assert x['msg'] == 'ensure this value has at least 1 characters'
            ...
            if x['loc'][-1] == 'va1_discount': assert x['msg'] == 'ensure this value is greater than or equal to 0'
        ...
        assert response.status_code == 422
        for x in response.json()['detail']:
            if x['loc'][-1] == 'va2_id': assert x['msg'] == 'ensure this value has at least 1 characters'
            if x['loc'][-1] == 'va2_option': assert x['msg'] == 'ensure this value has at least 1 characters'
            if x['loc'][-1] == 'va2_price': assert x['msg'] == 'ensure this value has at least 1 characters'
            if x['loc'][-1] == 'va2_stock': assert x['msg'] == 'ensure this value has at least 1 characters'
            ...
            if x['loc'][-1] == 'va2_discount': assert x['msg'] == 'ensure this value is greater than or equal to 0'
        ...
        for x in response.json()['detail']:
            ...
            if x['loc'][-1] == 'va1_discount': assert x['msg'] == 'ensure this value is less than or equal to 95'
        ...
        for x in response.json()['detail']:
            ...
            if x['loc'][-1] == 'va2_discount': assert x['msg'] == 'ensure this value is less than or equal to 95'
        ...
        for x in response.json()['detail']:
            ...
            if x['loc'][-1] == 'va1_product_id': assert x['msg'] == 'str type expected'
        ...
        response = client.post(url,json={
            'va1_items': [{
                ...
                'va1_barcode': 123,
                'va1_discount': '123',
                'va1_discount_active': 123,
                'va2_items': {}
            }]
        })
        assert response.status_code == 422
        for x in response.json()['detail']:
            if x['loc'][-1] == 'va1_id': assert x['msg'] == 'str type expected'
            if x['loc'][-1] == 'va1_option': assert x['msg'] == 'str type expected'
            if x['loc'][-1] == 'va1_price': assert x['msg'] == 'str type expected'
            if x['loc'][-1] == 'va1_stock': assert x['msg'] == 'str type expected'
            if x['loc'][-1] == 'va1_code': assert x['msg'] == 'str type expected'
            if x['loc'][-1] == 'va1_barcode': assert x['msg'] == 'str type expected'
            if x['loc'][-1] == 'va1_discount': assert x['msg'] == 'value is not a valid integer'
            if x['loc'][-1] == 'va1_discount_active': assert x['msg'] == 'value is not a valid boolean'
        ...
        response = client.post(url,json={
            'va1_items': [{
                'va2_items': [{
                    ...
                    'va2_barcode': 123,
                    'va2_discount': '123',
                    'va2_discount_active': 123
                }]
            }]
        })
        assert response.status_code == 422
        for x in response.json()['detail']:
            if x['loc'][-1] == 'va2_id': assert x['msg'] == 'str type expected'
            if x['loc'][-1] == 'va2_option': assert x['msg'] == 'str type expected'
            if x['loc'][-1] == 'va2_price': assert x['msg'] == 'str type expected'
            if x['loc'][-1] == 'va2_stock': assert x['msg'] == 'str type expected'
            if x['loc'][-1] == 'va2_code': assert x['msg'] == 'str type expected'
            if x['loc'][-1] == 'va2_barcode': assert x['msg'] == 'str type expected'
            if x['loc'][-1] == 'va2_discount': assert x['msg'] == 'value is not a valid integer'
            if x['loc'][-1] == 'va2_discount_active': assert x['msg'] == 'value is not a valid boolean'
        ...
        assert response.status_code == 422
        for x in response.json()['detail']:
            if x['loc'][-1] == 'va1_product_id': assert x['msg'] == 'string does not match regex \"^[0-9]*$\"'
        ...
        assert response.status_code == 422
        for x in response.json()['detail']:
            if x['loc'][-1] == 'va1_id': assert x['msg'] == 'string does not match regex \"^[0-9]*$\"'
            if x['loc'][-1] == 'va1_price': assert x['msg'] == 'string does not match regex \"^[0-9]*$\"'
            if x['loc'][-1] == 'va1_stock': assert x['msg'] == 'string does not match regex \"^[0-9]*$\"'
        ...
        assert response.status_code == 422
        for x in response.json()['detail']:
            if x['loc'][-1] == 'va2_id': assert x['msg'] == 'string does not match regex \"^[0-9]*$\"'
            if x['loc'][-1] == 'va2_price': assert x['msg'] == 'string does not match regex \"^[0-9]*$\"'
            if x['loc'][-1] == 'va2_stock': assert x['msg'] == 'string does not match regex \"^[0-9]*$\"'
        ...
        response = client.post(url,json={'va1_items': [{'va1_price': 1, 'va1_stock': 0, 'va1_discount': 0}]})
        assert response.status_code == 422
        for x in response.json()['detail']:
            if x['loc'][-1] == 'va1_items': assert x['msg'] == 'ensure va1_discount_active value is not null'

        response = client.post(url,json={'va1_items': [{'va1_price': 1, 'va1_stock': 0, 'va1_discount_active': False}]})
        assert response.status_code == 422
        for x in response.json()['detail']:
            if x['loc'][-1] == 'va1_items': assert x['msg'] == 'ensure va1_discount value is not null'

        response = client.post(url,json={'va1_items': [{'va1_price': 1, 'va1_stock': 0, 'va1_discount': 0,'va1_discount_active': True}]})
        assert response.status_code == 422
        for x in response.json()['detail']:
            if x['loc'][-1] == 'va1_items': assert x['msg'] == 'ensure va1_discount value is greater than 0'

        response = client.post(url,json={'va1_items': [{'va1_id': 0,'va1_price': 1, 'va1_stock': 0}]})
        assert response.status_code == 422
        for x in response.json()['detail']:
        ...
        response = client.post(url,json={
            'va1_name': 'ukuran',
            'va1_items': [{'va1_option': 'XL','va1_price': 1, 'va1_stock': 0, 'va1_discount': 0}]
        })
        assert response.status_code == 422
        for x in response.json()['detail']:
            if x['loc'][-1] == 'va1_items': assert x['msg'] == 'ensure va1_discount_active at index 0 value is not null'

        response = client.post(url,json={
            'va1_name': 'ukuran',
            'va1_items': [{'va1_option': 'XL','va1_price': 1, 'va1_stock': 0, 'va1_discount_active': False}]
        })
        assert response.status_code == 422
        for x in response.json()['detail']:
            if x['loc'][-1] == 'va1_items': assert x['msg'] == 'ensure va1_discount at index 0 value is not null'

        response = client.post(url,json={
            'va1_name': 'ukuran',
            'va1_items': [{'va1_option': 'XL','va1_price': 1, 'va1_stock': 0, 'va1_discount': 0,'va1_discount_active': True}]
        })
        assert response.status_code == 422
        for x in response.json()['detail']:
        ...
        response = client.post(url,json={
            'va1_name': 'ukuran',
            'va1_items': [{'va1_id': 0,'va1_option': 'XL','va1_price': 1, 'va1_stock': 0}]
        })
        assert response.status_code == 422
        for x in response.json()['detail']:
            if x['loc'][-1] == 'va1_items': assert x['msg'] == 'ensure va1_product_id value is not null'
        ...
        response = client.post(url,json={
            ...
            'va1_items': [{
                'va1_option': 'Hitam',
                'va2_items': [{'va2_option':'XL', 'va2_price': 1, 'va2_stock': 0, 'va2_discount': 0}]
            }]
        })
        assert response.status_code == 422
        for x in response.json()['detail']:
            if x['loc'][-1] == 'va1_items': assert x['msg'] == 'ensure va2_discount_active at option Hitam and index 0 value is not null'
        ...
        response = client.post(url,json={
            ...
            'va1_items': [{
                'va1_option': 'Hitam',
                'va2_items': [{'va2_option':'XL', 'va2_price': 1, 'va2_stock': 0, 'va2_discount_active': False}]
            }]
        })
        assert response.status_code == 422
        for x in response.json()['detail']:
            if x['loc'][-1] == 'va1_items': assert x['msg'] == 'ensure va2_discount at option Hitam and index 0 value is not null'
        ...
        response = client.post(url,json={
            ...
            'va1_items': [{
                'va1_option': 'Hitam',
                'va2_items': [{'va2_option':'XL', 'va2_price': 1, 'va2_stock': 0, 'va2_discount': 0, 'va2_discount_active': True}]
            }]
        })
        assert response.status_code == 422
        for x in response.json()['detail']:
        ...
        response = client.post(url,json={
            ...
            'va1_items': [{
                'va1_option': 'Hitam',
                'va2_items': [{'va2_id': 0,'va2_option':'XL', 'va2_price': 1, 'va2_stock': 0}]
            }]
        })
        assert response.status_code == 422
        for x in response.json()['detail']:
